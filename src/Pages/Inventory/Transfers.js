...
export default function Transfer() {
    ...
    async function handleDeleteTransfer() {
        ...
        if(filterConfig.tab === "on_hold") {
            if (transferType === "Mango") {
                const cash = await deleteTransferReceive(transferId);
                console.log(cash)
                ...
                const cash = await deleteTransferReceivePotato(transferId);
                if (cash.data.response === "Transfer deleted successfully") {
                    toast.success("Transfer deleted successfully", {
                        style: toastStyle(),
                    });
    ...
    async function handleSelectChange(e, idno, id, status, row, type) {
        console.log(e, idno, id, status, type)
        console.log(row)
        switch (e.target.value) {
            ...
            case "approve-po":
                window.open(
                    "/transfers/receive/update/" + id , "_blank"
                );
                break;
    ...
    const [filterConfig, setFilterConfig] = useState({
        tab: state === null ? "processed,for_adjustment" : state.tab,
        status: state === null ? "processed,for_adjustment" : state.tab,
        ...
        if(filterConfig.tab !== "on_hold") {
            ...
            if (filterConfig.shop_type !== "potato_corner" && response.data) {
                ...
                mangoTransfers.map(async (transfer) => {
                    ...
                    info.id = transfer.id;
                    info.status = transfer.status !== "on_hold" ? transfer.status : "On hold";
                    info.to_receive = transfer.to_receive;
            ...
            const response = await searchTransferReceive(filterConfig);
            const response2 = await searchTransferReceivePotato(filterConfig);
            ...
            if (filterConfig.shop_type !== "potato_corner" && response.data) {
                ...
                mangoTransfers.map(async (transfer) => {
                    ...
                    info.id = transfer.id;
                    info.transfer_id = transfer.transfer_id;
                    info.status = transfer.status !== "on_hold" ? transfer.status : "On hold";
                    info.to_receive = transfer.to_receive;
                    info.added_on = Moment(transfer.added_on).format("MM-DD-YYYY");
                    info.transfer_date = Moment(transfer.transfer_date).format("MM-DD-YYYY");
        ...
        if (tabKey === "") {
            setFilterConfig((prev) => {
                return {
                    ...
                    branch_from: "",
                    branch_to: "",
                    transfer_date_from: new Date(date),
                    transfer_date_to: new Date(nextDay),
        ...
        if(row.to_receive === "1") {
            return (
                <button
                    ...
                    className="btn btn-sm view-btn-table"
                    id={row.id}
                    onClick={(e) => handleReceiveTransfer(row.doc_no, row.status)}
                    value="receive-po"
                >
                    Receive
                </button>
    ...
    function handleReceiveTransfer(id, transfer_status) {
        window.open(
            "/transfers/receive/create/" + id , "_blank"
        );
    }

    function ReviewOnHold(row, type) {
        return (
            <button
                name="action"
                className="btn btn-sm view-btn-table"
                id={row.id}
                onClick={(e) => handleReviewOnHold(row.doc_no, row.status)}
                value="receive-po"
            >
                Approve
            </button>
    ...
    function handleReviewOnHold(id, transfer_status) {
        window.open(
            "/transfers/receive/update/" + id , "_blank"
        );
    }
    ...
    function ActionBtn(row, type) {
        ...
        return (
            <Form.Select
                ...
                id={row.id}
                onChange={(e) =>
                    handleSelectChange(e, row.id, row.doc_no, row.status, row, type)
                }
            >
                ...
                >
                    ...
                    <Tab eventKey="processed,for_adjustment" title="processed">
                        ...
                        <div className="my-2 px-4 PO-filters d-flex">
                            ...
                            <DatePicker
                                selected={filterConfig.transfer_date_from}
                                placeholderText={"Select Date"}
                                name="transfer_date_from"
                                onChange={(date) => {
                                    setFilterConfig((prev) => {
                                        return { ...prev, transfer_date_from: date };
                                    });
                                }}
                                fixedHeight
                            ...
                            <DatePicker
                                selected={filterConfig.transfer_date_to}
                                placeholderText={"Select Date"}
                                name="transfer_date_to"
                                onChange={(date) => {
                                    setFilterConfig((prev) => {
                                        return { ...prev, transfer_date_to: date };
                                    });
                                }}
                        ...
                        <Table
                            ...
                            PendingBtn={(row) => ReceiveBtn(row)}
                            showLoader={showLoader}
                        />
                        <div className="mb-2" />
                    </Tab>

                    <Tab eventKey="on_hold" title="on hold">
                        {/* filters */}
                        <div className="my-2 px-4 PO-filters d-flex">
                            <span className="me-4 align-middle mt-2">
                                Filter By:
                        ...
                        <Table
                            ...
                            tableData={transferManager}
                            ActionBtn={(row) => ActionBtn(row, "on_hold")}
                            ViewBtn={(row) => ReviewOnHold(row)}
                            // PendingBtn={(row) => ReceiveBtn(row)}
                            showLoader={showLoader}
                        />
                        <div className="mb-2" />
